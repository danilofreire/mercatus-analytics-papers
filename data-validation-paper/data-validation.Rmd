...
Content validity refers to whether the variables correspond to the abstract
theoretical concept it intends to measure. This is a difficult task as there
are no hard-and-fast rules on how to check this measure of data validity. Here
I follow Gerring [-@gerring1999makes; -@gerring2001social] and suggest that
...
*Resonance* means that the variable name reflects an appropriate understanding
of the concept. A clear definition is very often a simple word that is already
is use in common language, and that immediately evokes an intuitive
understanding of the idea the authors are trying to convey. They are akin to
mnemonic devices, in a way that readers can remember what the variable means
long after they see it [@gerring1999makes, 370]. Terms like "social capital" or
"civic culture" might be difficult to measure, but they do resonate with 
the general public [@bjornskov2013social].
...
researcher has a firm idea of what are the main characteristics of a given
concept for their target public.

*Differentiation* indicates that the variable should exclude information that
do not conform to the concept of interest. In other words, it entails that one
should find what makes a concept distinct from other related terms, and the
sharper those boundaries are, the stronger the validity of the concept. As
@gerring1999makes [375] notes, any good definition of "state" has to single out
what is particular to the state and which does not appear in other forms of
social organisation, such as tribes or empires. Many concepts in political
science and economics, such as "conflict" or "utility", cause confusion because
of "conceptual stretch", that is, "[...] an unreflecting application of
concepts to new issues" that do not conform to the original formulation
[@carlsson2017policy, 151; @sartori1970concept].

Similarly, *fecundity* indicates that the variable is parsimonious and include
only the necessary information relevant to the concept. It refines the
...
suggestion may be to start with competing definitions or with characteristics
that can conflate the original idea with similar behaviour. For instance, a
corruption variable should exclude private benefits that do not originate from
someone's governmental position [@mcmann2016strategies, 10].

*Consistency* is an attribute that signals whether a concept retain its
validity across different settings [@mcmann2016strategies, 12]. An indicator of
liberal democracy should be able to explain not only Western regimes, but be
able to identify liberal traits in countries that do not share a similar
background and retain its validity over time. 

Lastly, *causal utility* refers to the idea that the measure is useful to
specific areas of study, which the authors should enumerate when constructing
their data.
