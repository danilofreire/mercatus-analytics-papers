...
combined and for what reason. Moreover, one can aggregate features with
multiple functional forms. Aggregation rules should be theoretically driven,
and it is unclear whether additivity, the default method for merging low-level
attributes, is the appropriate rule for every case [@munck2002conceptualizing,
24]. 

One suggestion is to use Bayesian factor analysis to merge the
low-level indicators, as Bayesian inference adds uncertainty to the resulting
index [@mcmann2016strategies, 15]. 
