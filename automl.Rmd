...
has a `export` function that is useful for those who need to export the optimised model and deploy it in other settings. Users can also customise `TPOT`'s hyperparameters for classification and regression tasks. `TPOT`'s documentation is available at <http://epistasislab.github.io/tpot/> and its source code is hosted at <https://github.com/epistasislab/tpot/>.
...
Here I replicate two widely-cited papers that apply machine learning to forecast
rare events. The first paper is "Comparing Random Forest with Logistic
Regression for Predicting Class-Imbalanced Civil War Onset Data" by
@muchlinski2016comparing, and 
