...
predictor = task.fit(data_train, label=dependent_variable) # fit model
prediction = predictor.predict(new_data)                   # predict
```

Experienced machine learning practitioners may also adapt the `fit()` function
to their needs. For instance, one can choose different learners to estimate
the models -- random forests, 
